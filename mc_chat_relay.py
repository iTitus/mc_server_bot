...
def _follow(log_file_path: str) -> Generator[str, None, None]:
    ...
    while True:
        ...
        if not line:
            ...
            if pos != size:  # log file size changed without reading -> new server instance started
        ...
        if match:
            ...
            try:
                if me_opener and chat_msg:
                    webhook.send('*{}*'.format(util.escape_all(chat_msg)), username=name,
                                 avatar_url='https://minotar.net/helm/{}/256.png'.format(name))
                elif chat_opener and chat_msg:
                    webhook.send(util.escape_all(chat_msg), username=name,
                                 avatar_url='https://minotar.net/helm/{}/256.png'.format(name))
                elif msg:
                ...
                for tb_line in ''.join(traceback.format_exception(type(e), e, e.__traceback__)).splitlines():
                    log.error(tb_line)
        else:
...
class McChatRelay(commands.Cog):
    ...
    async def __send_to_mc(self, message: discord.Message):
        ...
        try:
            ...
            log.error('Error while sending RCON message: {}: {}'.format(message.author, message.clean_content))
            for tb_line in ''.join(traceback.format_exception(type(e), e, e.__traceback__)).splitlines():
                log.error(tb_line)
